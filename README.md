...
REPL 斜杠命令：`/help` `/reset` `/undo` `/tools` `/status` `/model` `/setting` `/think on|off` `/mascot` `/skill` `/vim` `/quit`。Enter 发送，行末 `\` 续行。Tab 在输入、文件、Changes 之间循环；F1 / F2 / Ctrl+T 切换对话与文本标签。当前模型没有 thinking 时不显示 `/think`。
