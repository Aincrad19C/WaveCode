...
- **模型**：DeepSeek 官方 API。
  ...
  - 无 thinking 的模型：`/help`、`/status` 不显示 thinking 开关。工作区 HUD 从不显示 thinking。
- **模式**：`ask` / `plan` / `agent`（见 [14-modes.md](./14-modes.md)）。默认 agent。输入框最前着色显示；`/mode` 切换。plan 每次只问一个问题，确认后留下内部 `# 计划` 文档。
- **API**：
